...
<script type="text/javascript" src="${pageContext.request.contextPath }/resources/js/crew-update.js"></script>
</head>
...
<div class="col-lg-12 ftco-animate" style="padding: 0px 0px 60px">
  <div class="container" style="padding: 0px 0px 60px">
		...
		<input type="hidden" name="crewNum" value="${dto.crewNum}">
		<input type="hidden" name="userId" value="${dto.userId}" readonly>
		<div>
			
			<select class="form-select" id="actLocation" name="actLocation">
				<option selected>${dto.actLocation}</option>
				
					<c:forEach var="areaList" items="${areaList}" >
						<c:if test="${areaList.area != dto.actLocation}">
			...
			<select class="form-select" id="actCategory" name="actCategory" onchange="actSelect()">
				<option selected>${dto.actCategory}</option>
			</select>
			<select class="form-select" id="actTitle" name="actTitle">
				<option selected>${dto.actTitle}</option>
			</select>
			</span>
		</div>
		<div style="margin-top: 0.5em;">
			<input type="text" id="crewSubject" class="c-sub" name="crewSubject" placeholder="제목을 입력해 주세요." value="${dto.crewSubject}">
		</div>
		<div class="content" style="margin-top: 0.5em;">
			<textarea class="txt-a" id="crewContent" name="crewContent" rows="15" placeholder="내용을 입력해 주세요.">${dto.crewContent}</textarea>
