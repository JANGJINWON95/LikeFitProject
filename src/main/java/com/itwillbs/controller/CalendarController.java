...
public class CalendarController {
	...
	public String calendar(Model model, HttpServletRequest request, HttpSession session){
		
		DateDTO dateDTO = new DateDTO();
		dateDTO.setUserId(session.getAttribute("userId").toString());
		System.out.println("id값 들어왔다 : "+dateDTO.getUserId());
...
//	ArrayList<ScheduleDTO> Schedule_list =  scheduleDao.schedule_list(dateData);
	...
	@RequestMapping(value = "/scheduleAdd", method = RequestMethod.GET)
		public String schedule_add(HttpServletRequest request, ScheduleDTO scheduleDTO, RedirectAttributes rttr, HttpSession session){
		
		scheduleDTO.setUserId(session.getAttribute("userId").toString());
		
		
		System.out.println("컨트롤러 : " +scheduleDTO);
